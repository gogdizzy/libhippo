...
int createTimerfd() {
	...
	if( timerfd < 0 ) {
		HIPPO_ERROR( "Failed in timerfd_create" );
	}
	return timerfd;
#else
	return 0;
...
void readTimerfd( int timerfd, Timestamp now ) {
	...
	HIPPO_DEBUG( "TimerQueue::handleRead() {} at {}", howmany, now.toString() );
	if( n != sizeof howmany ) {
		HIPPO_ERROR( "TimerQueue::handleRead() reads {} bytes instead of 8", n );
	}
}
