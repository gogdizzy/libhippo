...
int sockets::createNonblockingOrDie() {
	...
	if( sockfd < 0 ) {
		HIPPO_ERROR( "sockets::createNonblockingOrDie" );
	}
...
void sockets::listenOrDie( int sockfd ) {
	...
	if( ret < 0 ) {
		HIPPO_ERROR( "sockets::listenOrDie" );
	}
}
...
int sockets::accept( int sockfd, struct sockaddr_in *addr ) {
	...
	if( connfd < 0 ) {
		...
		switch( savedErrno ) {
			...
			case EOPNOTSUPP:
				// unexpected errors
				HIPPO_ERROR( "unexpected error of ::accept {}", savedErrno );
				break;
			default:
				HIPPO_ERROR( "unknown error of ::accept {}", savedErrno );
				break;
		}
	}
	return connfd;
...
void sockets::close( int sockfd ) {
	if( ::close( sockfd ) < 0 ) {
		HIPPO_ERROR( "sockets::close" );
	}
}

void sockets::shutdownWrite( int sockfd ) {
	if( ::shutdown( sockfd, SHUT_WR ) < 0 ) {
		HIPPO_ERROR( "sockets::shutdownWrite" );
	}
}
...
void sockets::fromIpPort( const char *ip, uint16_t port,
	...
	if( ::inet_pton( AF_INET, ip, &addr->sin_addr ) <= 0 ) {
		HIPPO_ERROR( "sockets::fromIpPort" );
	}
}
...
struct sockaddr_in sockets::getLocalAddr( int sockfd ) {
	...
	if( ::getsockname( sockfd, sockaddr_cast( &localaddr ), &addrlen ) < 0 ) {
		HIPPO_ERROR( "sockets::getLocalAddr" );
	}
	return localaddr;
}
...
struct sockaddr_in sockets::getPeerAddr( int sockfd ) {
	...
	if( ::getpeername( sockfd, sockaddr_cast( &peeraddr ), &addrlen ) < 0 ) {
		HIPPO_ERROR( "sockets::getPeerAddr" );
	}
	return peeraddr;
}
